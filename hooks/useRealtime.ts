...
export function useRealtime({
  ...
  const broadcastObjectCreated = useCallback(
    async (object: CanvasObject, userId?: string, displayName?: string) => {
      ...
      if (!channelRef.current || !effectiveUser) {
        console.log(
          "🚨 DEBUG: broadcastObjectCreated skipped - missing channel/user"
        );
        return;
      }
  ...
  const broadcastObjectUpdated = useCallback(
    async (object: CanvasObject, userId?: string) => {
      ...
      if (!channelRef.current || !effectiveUser) {
        console.log(
          "🚨 DEBUG: broadcastObjectUpdated skipped - missing channel/user"
        );
        return;
      }
  ...
  const broadcastObjectDeleted = useCallback(
  ...
  const broadcastObjectsDeleted = useCallback(
  ...
  const broadcastObjectsDuplicated = useCallback(
    async (originalIds: string[], newObjects: CanvasObject[]) => {
      if (!channelRef.current || !user || !profile) return;

      if (isDev) {
        console.log(
          "📡 Broadcasting objects duplicated:",
          originalIds.length,
          "objects by",
  ...
  const broadcastOwnershipClaimed = useCallback(
    async (event: {
      object_id: string;
      owner_id: string;
      owner_name: string;
  ...
  const broadcastOwnershipReleased = useCallback(
    async (event: {
      object_id: string;
  ...
  const broadcastOwnershipRejected = useCallback(
    async (event: {
      object_id: string;
  ...
  const broadcastCursorMoved = useCallback(
  ...
  const triggerReconnection = useCallback(() => {
    ...
    reconnectTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.log(
          `🔄 Attempting to reconnect channels (attempt ${reconnectAttemptsRef.current})...`
        );
  ...
  useEffect(() => {
    ...
    if (!user || !profile) {
      ...
      if (!hasLoggedWaitingRef.current) {
        console.log(
          "⏳ Waiting for user authentication before setting up realtime..."
        );
    ...
    const {
      ...
      if (event === "SIGNED_OUT" || !session) {
        ...
        console.log(
          "🔄 Token refreshed, channels should reconnect automatically..."
        );
    ...
    channel
      .on("broadcast", { event: "objects_deleted" }, (payload) => {
        ...
        if (onObjectsDeletedRef.current) {
          onObjectsDeletedRef.current(
            payload.payload as RealtimeEvents["objects_deleted"]
          );
        }
      })
      .on("broadcast", { event: "objects_duplicated" }, (payload) => {
        ...
        if (onObjectsDuplicatedRef.current) {
          onObjectsDuplicatedRef.current(
            payload.payload as RealtimeEvents["objects_duplicated"]
          );
        }
      })
      ...
      .on("broadcast", { event: "object_created" }, (payload) => {
        ...
        if (onObjectCreatedRef.current) {
          onObjectCreatedRef.current(
            payload.payload as RealtimeEvents["object_created"]
          );
        }
      })
      .on("broadcast", { event: "object_updated" }, (payload) => {
        ...
        if (onObjectUpdatedRef.current) {
          onObjectUpdatedRef.current(
            payload.payload as RealtimeEvents["object_updated"]
          );
        }
      })
      .on("broadcast", { event: "object_deleted" }, (payload) => {
        ...
        if (onObjectDeletedRef.current) {
          onObjectDeletedRef.current(
            payload.payload as RealtimeEvents["object_deleted"]
          );
        }
      })
        ...
        await Promise.all([
          channel.subscribe((status) => {
            ...
            setState((prev) => ({
              ...
              error:
                status === "CHANNEL_ERROR"
                  ? "Failed to connect to canvas channel"
                  : null,
            }));
            ...
            if (status === "CHANNEL_ERROR") {
              console.log(
                "🔄 Channel error detected, triggering reconnection..."
              );
              triggerReconnection();
            }
          }),
